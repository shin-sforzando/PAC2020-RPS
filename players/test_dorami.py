...
def test_hands(trials: int = 10000, tolerance: float = 0.1):
    ...
    for _ in range(trials):
        history.append(dorami.next_hand())
    ...
    assert isclose(trials * 0.50, num_g, rel_tol=tolerance)
    assert isclose(trials * 0.25, num_c, rel_tol=tolerance)
    assert isclose(trials * 0.25, num_p, rel_tol=tolerance)
