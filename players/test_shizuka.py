...
def test_hands(trials: int = 10000, tolerance: float = 0.1):
    ...
    assert isclose(trials / 3, len([h for h in history if h is Hand.G]), rel_tol=tolerance)
    assert isclose(trials / 3, len([h for h in history if h is Hand.C]), rel_tol=tolerance)
    assert isclose(trials / 3, len([h for h in history if h is Hand.P]), rel_tol=tolerance)
